...
def guard():
    ...
    if not check("Paystack key is TEST mode", secret.startswith("sk_test_"), secret[:8]):
        sys.exit(1)
    return secret
...
def post_webhook(secret, event, data, tamper=False):
    ...
    return call(
        ...
        raw=raw, headers={"X-Paystack-Signature": sig},
    )
...
def main():
    ...
    check("unauthenticated wallet read is rejected", status in (401, 403), f"status {status}")

    status, body = call("GET", "/users/profile", token=seller_token)
    check("bearer token authenticates /users/profile", status == 200, f"status {status}")
    ...
    check("feed is mixed (posts and products)", {"post", "product"} <= kinds, f"types={sorted(kinds)}")

    pag = feed.get("pagination", {}) if isinstance(feed, dict) else {}
    check("feed exposes pagination the app relies on",
          all(k in pag for k in ("page", "per_page", "has_next")), str(sorted(pag))[:120])

    ids = [i.get("id") for i in items]
    check("feed item ids are unique and present", len(ids) == len(set(ids)) and all(ids),
          f"{len(ids)} ids")
    ...
    if post:
        ...
        check("like persists (count incremented)", after == before + 1, f"{before} -> {after}")
        check("post detail returns liked_by_me for the liker",
              d.get("liked_by_me") is True, f"liked_by_me={d.get('liked_by_me')}")

        status, _ = call("POST", f"/socials/posts/{post['id']}/comments",
                         token=buyer_token, body={"content": "smoke-fixture: nice one"})
        ...
        check("comment persists (count incremented)", (cc or 0) >= 1, f"comment_count={cc}")
    else:
        check("post flows exercised", False, "no post in feed")

    if product:
        seller_ref = (product.get("seller") or {}).get("id")
        check("product carries seller.id for the shop tap", seller_ref is not None, f"seller.id={seller_ref}")
        ...
        status, _ = call("POST", "/cart/add", token=buyer_token,
    ...
    check("wallet balance readable", start_balance is not None, f"balance={start_balance}")

    status, body = call("POST", "/wallet/topup/initialize", token=seller_token,
    ...
    check("top-up callback route resolves (was 404)", status in (200, 302), f"status {status}")
    ...
    try:
        ...
        location = f"error: {e}"
    check("callback redirects to the mobile deep link",
          location.startswith("markt://wallet/"), location[:80])
    ...
    check("unpaid top-up is not credited", v.get("verified") is False, f"verified={v.get('verified')}")
    check("balance unchanged by an unpaid top-up",
    ...
    status, _ = post_webhook(secret, "charge.success",
                             {"reference": reference, "status": "success",
                              "amount": 500000, "currency": "NGN"}, tamper=True)
    check("tampered signature is rejected", status == 400, f"status {status}")
    check("balance unchanged after tampered webhook",
          get_balance(seller_token) == start_balance, f"balance={get_balance(seller_token)}")
    ...
    check("balance moved by exactly the reported amount",
          after_credit == round((start_balance or 0) + 5000, 2),
          f"{start_balance} -> {after_credit} (expected +5000)")

    # (c) replay
    for i in range(5):
        post_webhook(secret, "charge.success",
                     {"reference": reference, "status": "success",
                      "amount": 500000, "currency": "NGN"})
    replayed = get_balance(seller_token)
    check("5x replay does not move the balance again",
          replayed == after_credit, f"{after_credit} -> {replayed}")

    status, body = call("GET", "/wallet/transactions?page=1&per_page=50", token=seller_token)
    txs = unwrap(body).get("transactions", []) if status == 200 else []
    topup_rows = [t for t in txs if t.get("reference_type") == "wallet_topup"]
    check("exactly one ledger row for the top-up after replay",
          len(topup_rows) == 1, f"{len(topup_rows)} rows")
    check("ledger balance_after agrees with balance endpoint",
    ...
    for label, data in [
        ...
        st2, b2 = call("POST", "/wallet/topup/initialize", token=seller_token,
        ...
        check(f"signed webhook with {label} does not credit", pre == post, f"{pre} -> {post}")

    # (f) consistency
    section("Wallet — read-back consistency")
    bal = get_balance(seller_token)
    status, body = call("GET", "/wallet/transactions?page=1&per_page=100", token=seller_token)
    txs = unwrap(body).get("transactions", []) if status == 200 else []
    computed = 0.0
    for t in sorted(txs, key=lambda r: r["id"]):
        computed += t["amount"] if t["type"] == "credit" else -t["amount"]
    check("ledger sums to the reported balance", round(computed, 2) == round(bal, 2),
          f"sum={round(computed,2)} balance={bal}")

    status, body = call("GET", "/wallet/withdrawals?page=1&per_page=20", token=seller_token)
    ...
    check("clean account reports can_delete", dc.get("can_delete") is True, str(dc)[:120])

    status, _ = call("DELETE", "/users/account", token=dis_token,
    ...
    check("bad confirmation string is rejected", status in (400, 422), f"status {status}")

    status, body = call("DELETE", "/users/account", token=dis_token,
    ...
    check("pre-deletion bearer token no longer authenticates",
    ...
    status, _ = call("POST", "/cart/add", token=buyer_b_token,
                     body={"product_id": product["id"], "quantity": 1, "variant_id": 0})
    addr = {
        "recipient_name": "Smoke Buyer B", "street_address": "1 Smoke Lane",
        "city": "Lagos", "state": "Lagos", "postal_code": "100001",
        "country": "Nigeria", "phone_number": "+2348000000000",
    }
    status, body = call("POST", "/cart/checkout", token=buyer_b_token,
                        body={"shipping_address": addr, "billing_address": addr})
    order_created = status in (200, 201)
    check("checkout creates an order (for the blocker test)", order_created, f"status {status}")
    ...
    if order_created:
        ...
        check("account with an order in flight reports an open-order blocker",
              dc3.get("can_delete") is False and "open_orders_buying" in codes3,
              str(codes3))
        status, _ = call("DELETE", "/users/account", token=buyer_b_token,
                         body={"password": PASSWORD, "confirmation": "DELETE"})
        check("deletion is refused while an order is in flight", status == 409,
              f"status {status}")
    ...
    check("funded account reports a wallet blocker",
          dc2.get("can_delete") is False and "wallet_balance" in codes, str(codes))

    status, body = call("DELETE", "/users/account", token=seller_token,
                        body={"password": PASSWORD, "confirmation": "DELETE"})
    check("deletion of a funded account is refused with 409", status == 409, f"status {status}")
    status, _ = call("GET", "/users/profile", token=seller_token)
    check("refused deletion leaves the account usable", status == 200, f"status {status}")
